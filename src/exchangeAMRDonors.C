...
void tioga::exchangeAMRDonors()
{
    ...
    for (i = 0; i < nsend; i++) {
        ...
        sndPack[i].realData =
            (double*)malloc(sizeof(double) * sndPack[i].nreals);
    ...
    for (i = 0; i < nrecv; i++) {
        if (rcvPack[i].nreals > 0) {
            ...
            for (j = 0; j < interpCount; j++) {
                ...
                cb[localid].insertInInterpList(
                    i, remoteid, remoteblockid, xtmp);
                bcount[localid]++;
            }
            for (j = 0; j < donorCount; j++) {
                ...
                cb[localid].insertInDonorList(
                    i, index, meshtag, remoteid, remoteblockid, cellRes);
                bcount[localid]++;
            }
        }
    }
