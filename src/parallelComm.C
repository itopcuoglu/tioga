...
void parallelComm::sendRecvPacketsAll(PACKET* sndPack, PACKET* rcvPack) const
{
    ...
    int const all_snd_nints = std::accumulate(sint, sint + numprocs, 0);
    int const all_rcv_nints = std::accumulate(rint, rint + numprocs, 0);
    ...
    for (int i = 0; i < numprocs; i++) {
        int const displ = snd_int_displs[i];
    ...
    MPI_Ialltoallv(
        all_snd_intData, sint, snd_int_displs.data(), MPI_INT, all_rcv_intData,
        rint, rcv_int_displs.data(), MPI_INT, scomm, &int_request);

    int const all_snd_nreals = std::accumulate(sreal, sreal + numprocs, 0);
    int const all_rcv_nreals = std::accumulate(rreal, rreal + numprocs, 0);
    ...
    for (int i = 0; i < numprocs; i++) {
        int const displ = snd_real_displs[i];
    ...
    MPI_Ialltoallv(
        all_snd_realData, sreal, snd_real_displs.data(), MPI_DOUBLE,
...
void parallelComm::sendRecvPackets(PACKET* sndPack, PACKET* rcvPack)
{
    ...
    for (i = 0; i < nrecv; i++) {
        if (rcvPack[i].nints > 0) {
            ...
            rcvPack[i].intData = (int*)malloc(sizeof(int) * rcvPack[i].nints);
            MPI_Irecv(
                rcvPack[i].intData, rcvPack[i].nints, MPI_INT, rcvMap[i], tag,
                scomm, &request[irnum++]);
        }
        if (rcvPack[i].nreals > 0) {
            tag = 2;
            rcvPack[i].realData =
                (REAL*)malloc(sizeof(REAL) * rcvPack[i].nreals);
            MPI_Irecv(
                rcvPack[i].realData, rcvPack[i].nreals, MPI_DOUBLE, rcvMap[i],
                tag, scomm, &request[irnum++]);
        }
    }
    //
    for (i = 0; i < nsend; i++) {
        if (sndPack[i].nints > 0) {
            tag = 1;
            MPI_Isend(
                sndPack[i].intData, sndPack[i].nints, MPI_INT, sndMap[i], tag,
                scomm, &request[irnum++]);
        }
        if (sndPack[i].nreals > 0) {
            tag = 2;
            MPI_Isend(
                sndPack[i].realData, sndPack[i].nreals, MPI_DOUBLE, sndMap[i],
                tag, scomm, &request[irnum++]);
        }
    }
    MPI_Waitall(irnum, request, status);
...
void parallelComm::setMap(int ns, int nr, const int* snd, const int* rcv)
{
    int i;
    //
    if (sndMap != nullptr) TIOGA_FREE(sndMap);
    sndMap = nullptr;
    if (rcvMap != nullptr) TIOGA_FREE(rcvMap);
    rcvMap = nullptr;
    //
    nsend = ns;
    nrecv = nr;
...
void parallelComm::clearPackets(PACKET* sndPack, PACKET* rcvPack) const
{
    ...
    for (i = 0; i < nrecv; i++) {
        if (rcvPack[i].intData != nullptr) TIOGA_FREE(rcvPack[i].intData);
        if (rcvPack[i].realData != nullptr) TIOGA_FREE(rcvPack[i].realData);
