    ...
    double searchTol)
{
    ...
    for (int i = 0; i < nbodytags; i++) {
        compositeBody[compbodytag - BASE].bodyids[i] = meshtags[i];
    }
    for (int i = 0; i < nbodytags; i++) {
        compositeBody[compbodytag - BASE].dominanceflags[i] = dominancetags[i];
    }
}
...
void tioga::assembleCompositeMap()
{
    ...
    if (ncomposite > 0) {
        ...
        for (int mbi = 0; mbi < nblocks; mbi++) {
            ...
            for (int cb = 0; cb < ncomposite; cb++) {
                ...
                for (i = 0; i < nbodies; i++) {
                    ...
                    if (bodyi == mbtag) {
                        mb->setCompositeFlag(
                            Composite.searchTol, Composite.dominanceflags[i]);
                    }
                }
            }
        }
        ...
        // fill composite map: all ranks have compositeBody info ->
        // compositeBodyMap is globally known
        for (int cb = 0; cb < ncomposite; cb++) {
            CompositeBody& Composite = compositeBody[cb];
...
void tioga::assembleCompositeComms()
{
    ...
    for (int cb = 0; cb < ncomposite; cb++) {
        ...
        for (int i = 0; i < nbodies; i++) {
            ...
            MPI_Allreduce(
                MPI_IN_PLACE, &(MBC.masterID), 1, MPI_INT, MPI_MAX, scomm);
            ...
            MPI_Allgather(
                &foundFlag, 1, MPI_CHAR, existMeshTag.data(), 1, MPI_CHAR,
                scomm);
            ...
            MPI_Group_incl(
                scomm_group, compositecount, comp_list.data(),
                &meshblockCompositeGroup);
            ...
            // printf("My GID[%2d] (comp rank %2d of %2d), body_tag[%d] of %d
            // blocks, master rank %d, %d\n",
            //         myid,MBC.id,MBC.nrank,bodyi,maxtag,MBC.masterID,myid ==
            //         MBC.masterID);
...
void tioga::assembleComplementComms()
{
    ...
    for (int mbi = 0; mbi < nblocks; mbi++) {
        ...
        // cleanup memory
        if (mb->blockcomm != MPI_COMM_NULL) {
            MPI_Comm_free(&mb->blockcomm);
        }
    }
    MPI_Allreduce(&maxtagLocal, &maxtag, 1, MPI_INT, MPI_MAX, scomm);
    ...
    for (int mtag = 1; mtag <= maxtag; mtag++) {
        ...
        std::fill(existMeshTag.begin(), existMeshTag.end(), 0);
        MPI_Allgather(
            &foundFlag, 1, MPI_CHAR, existMeshTag.data(), 1, MPI_CHAR, scomm);
        ...
        // store mesh rank master in exclude list (as it broadcasts the hole
        // map)
        ...
        MPI_Group_incl(
            scomm_group, incount, incl_list.data(), &meshblockIncludeGroup);
        MPI_Group_incl(
            scomm_group, excount, comp_list.data(), &meshblockExcludeGroup);
        ...
        if (compListFlag != 0) {
            ...
            // printf("My GID[%2d] (comp rank %2d of %2d), body_tag[%d] of %d
            // blocks, master rank %d, %d\n",
            //         myid,MBC.id,MBC.nrank,mtag,maxtag,MBC.masterID,myid ==
            //         meshMasterId);
        }
        ...
        for (int mbi = 0; mbi < nblocks; mbi++) {
            ...
            if (mytag[mbi] == mtag) {
                ...
                MPI_Comm_create_group(
                    scomm, meshblockIncludeGroup, 0, &(mb->blockcomm));
                MPI_Comm_rank(mb->blockcomm, &mb->blockcomm_id);
                MPI_Comm_size(mb->blockcomm, &mb->blockcomm_numprocs);

                // Task 2a: set rank master for this body tag: lowest rank in
                // this block
                MPI_Allreduce(
    ...
    uint64_t* node_gid)
{
    ...
    mb->setData(
        btag, nnodes, xyz, ibl, nwbc, nobc, wbcnode, obcnode, ntypes, nv, nc,
        vconn, cell_gid, node_gid);
    mb->myid = myid;
}
...
void tioga::performConnectivityAMR()
{
    ...
    for (int ib = 0; ib < nbmax; ib++) {
        ...
        if (ib < nblocks) {
            mblocks[ib]->getCartReceptors(cg, pc_cart);
        }
        this->myTimer("tioga::getCartReceptors", 1);
        if (ib < nblocks) {
            mblocks[ib]->ihigh = ihigh;
        }
        ...
        if (ib < nblocks) {
            mblocks[ib]->getUnresolvedMandatoryReceptors();
        }
...
void tioga::dataUpdate_AMR()
{
    if ((nblocks > 0) && (ncart > 0)) {
        assert(
            mblocks[0]->num_var() ==
            (cb[0].num_cell_var() + cb[0].num_node_var()));
    }
    ...
    for (int ib = 0; ib < nblocks; ib++) {
        auto& mb = mblocks[ib];
        mb->getInterpolatedSolutionAMR(
            &nints, &nreals, &integerRecords, &realRecords, qblock[ib],
            sndMapNB);
    }
    for (i = 0; i < ncart; i++) {
        cb[i].getInterpolatedData(
            &nints, &nreals, &integerRecords, &realRecords);
    }
    //
    ...
    for (k = 0; k < nsend; k++) {
        ...
        sndPack[k].realData =
            (double*)malloc(sizeof(double) * sndPack[k].nreals);
        icount[k] = dcount[k] = 0;
    }
    ...
    for (k = 0; k < nrecv; k++) {
        ...
        for (i = 0; i < rcvPack[k].nints / 2; i++) {
            ...
            if (bid < 0) {
                ...
                cb[bid - 1].update(
                    &rcvPack[k].realData[m], rcvPack[k].intData[2 * i + 1]);
            }
            m += nvar;
        }
    }
...
void tioga::dataUpdate(int nvar, int interptype, int at_points)
{
    ...
    for (int ib = 0; ib < nblocks; ib++) {
        ...
        double* q = qblock[ib];
        if (at_points == 0) {
            mb->getInterpolatedSolution(
                &(nints[ib]), &(nreals[ib]), &(integerRecords[ib]),
                &(realRecords[ib]), q, nvar, interptype);
        } else {
            mb->getInterpolatedSolutionAtPoints(
                &(nints[ib]), &(nreals[ib]), &(integerRecords[ib]),
                &(realRecords[ib]), q, nvar, interptype);
        }
        //
    ...
    if (at_points != 0) {
        ...
        for (int ib = 0; ib < nblocks; ib++) {
            qtmp[ib] = (double*)malloc(
                sizeof(double) * mblocks[ib]->ntotalPoints * nvar);
            itmp[ib] = (int*)malloc(sizeof(int) * mblocks[ib]->ntotalPoints);
            for (int i = 0; i < mblocks[ib]->ntotalPoints; i++) {
                itmp[ib][i] = 0;
            }
        }
    }
...
void tioga::writeData(int nvar, int interptype)
{
    // mb->writeGridFile(myid);
    for (int ib = 0; ib < nblocks; ib++) {
        mblocks[ib]->writeFlowFile(
            100 * myid + ib, qblock[ib], nvar, interptype);
...
void tioga::getReceptorInfo(std::vector<int>& receptors)
{
    ...
    for (int ib = 0; ib < nblocks; ib++) {
        ...
        //
        // The donor cell GID is uint64_t and is packed using 2*sizeof(int)
...
void tioga::register_amr_local_data(
...
void tioga::preprocess_amr_data(int root)
{
    ...
    // Some MPI ranks do not have information regarding the AMR mesh. We
    // broadcast the AMR patch information from ranks that have it and then
    // create a valid CartGrid instance on MPI ranks that do not have it.
    ...
    // is similar to what CartGrid uses for the registerData interface. We add
